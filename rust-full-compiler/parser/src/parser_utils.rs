...
impl Parser {
  ...
  fn parse_fn_decl(&mut self, engine: &mut DiagnosticEngine) -> Result<Stmt, ()> {
    ...
    if !matches!(name.kind, TokenKind::Identifier) {
      let diagnostic = Diagnostic::new(
        DiagnosticCode::Error(DiagnosticError::InvalidFnName),
        "Invalid function name".to_string(),
        "duck.lox".to_string(),
      )
      .with_label(
        Span::new(name.span.line + 1, name.span.col + 1, name.lexeme.len()),
        Some("Invalid function name".to_string()),
        LabelStyle::Primary,
      );
    ...
    println!("parsing fn decl {:?}", self.current_token());
  ...
  fn parse_for_stmt(&mut self, engine: &mut DiagnosticEngine) -> Result<Stmt, ()> {
    ...
    let variable = if self.current_token().kind == TokenKind::Underscore {
    ...
    let iterator = self.parse_expr_stmt_with_context(engine, ExprContext::WhileCondition)?;
  ...
  fn parse_expr_stmt_with_context(
    ...
    self.parse_assignment(engine, context)
  }
